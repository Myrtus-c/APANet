import argparse
from baseline import PYG_ChebNet
...
def test(data, model, criterion, checkpt_file):
    ...
    with torch.no_grad():
        ...
        fpr, tpr, t = roc_curve(data.y[idx_test].detach().cpu().numpy(), pred_test.detach().cpu().numpy(),
                                pos_label=1)
        roc_auc = auc(fpr, tpr)
        return loss_test.item(), f1_test, precision_test, recall_test, roc_auc
...
def main():
    ...
    parser.add_argument('--K', type=int, default=3, help='Cheb Layers')
    ...
    net = PYG_ChebNet(in_channels=data.x.shape[1], out_channels=args.hidden, num_class=2, K=args.K)
    net.to(device)

    optimizer = optim.Adam(net.parameters(), lr=args.lr, weight_decay=args.wd)
    checkpt_file = 'pt/model_{}_2.pt'.format(args.dataset)
    # weights = torch.Tensor([10., ((data.y == 0).sum() / (data.y == 1).sum()).item()])
